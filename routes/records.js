...
/* GET users listing max */
router.get('/maximum', async function(req, res, next) {
  try {
    let queryParameters = {};
    ...
    if (sportParam) {
      ...
      response = response.reduce((accumulator, current) => {
        return accumulator.max > current.max ? accumulator : current;
      });

    }
    else {
      response = await Record.find(queryParameters).sort({"field":-1}).limit(1);
    }
...
router.post('/', async function(req, res, next) {
  try {
    const newRecord = new Record({userId: req.body.userId, exerciseType: req.body.exerciseType, side: req.body.side, max: req.body.max, avg: req.body.avg, data: req.body.data, date: req.body.date ? new Date(req.body.data) : new Date()});
